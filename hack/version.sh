...
function tim::version::get_version_vars() {
...
function tim::version::ldflag() {
  local key=${1}
  local val=${2}

  echo "-X 'github.com/tidbops/tim/pkg/version.${key}=${val}'"
}

# Prints the value that needs to be passed to the -ldflags parameter of go build
function tim::version::ldflags() {
  tim::version::get_version_vars

  local buildDate=
  [[ -z ${SOURCE_DATE_EPOCH-} ]] || buildDate="--date=@${SOURCE_DATE_EPOCH}"
  local -a ldflags=($(tim::version::ldflag "buildDate" "$(date ${buildDate} -u +'%Y-%m-%dT%H:%M:%SZ')"))
  if [[ -n ${GIT_COMMIT-} ]]; then
    ldflags+=($(tim::version::ldflag "gitCommit" "${GIT_COMMIT}"))
    ldflags+=($(tim::version::ldflag "gitTreeState" "${GIT_TREE_STATE}"))
  fi

  if [[ -n ${GIT_VERSION-} ]]; then
    ldflags+=($(tim::version::ldflag "gitVersion" "${GIT_VERSION}"))
  fi
